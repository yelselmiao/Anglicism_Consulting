...
```{r remove p10 in part 1}
data <- data %>% select(-p1_10)
```
...
p1_data_wide <- data %>%
  select(-c(15:24)) %>%
  mutate(id = 1:nrow(data), 
         score_sum = rowSums(.[1:14]),
         score_mean = rowMeans(.[1:14])) %>% 
  select(-c(1:14)) 
...
p1_data_long %>% 
  ...
  mutate(gender = ifelse(gender == 'Woman', 'Female', 'Male')) %>% 
  group_by(gender) %>%
...
p1_data_long %>% 
  ...
  geom_violin() + 
  labs(x = 'Place Growing up', y = 'Score', color = 'Place of Residence') + 
  theme(axis.title.x = element_text(size = 15,face = 'bold'),
        axis.title.y = element_text(size = 15,face = 'bold'),
        axis.text.y = element_text(size = 15),
        axis.text.x = element_text(size = 15))
...
p1_data_long %>% 
  ...
  geom_violin() + 
  labs(x = 'Place Living in', y = 'Score', color = 'Place of Residence') + 
  theme(axis.title.x = element_text(size = 15,face = 'bold'),
        axis.title.y = element_text(size = 15,face = 'bold'),
        axis.text.y = element_text(size = 15),
        axis.text.x = element_text(size = 15))
...
p1_data_wide %>% 
  ...
  select(group1, group2, estimate1, estimate2, estimate, p.adj, p.adj.signif)
```

```{r score vs. education level, warning = FALSE}
p1_data_long$edu_level <- recode_factor(p1_data_long$edu_level, 
                                        `General and professional teaching college (DEC, AEC)` = "Cegep (DEC, AEC)", 
                                        `Professional training (DEP, ASP, AEP)` = "Vocational Education (DEP, ASP, AEP)", 
                                        `High school` = "High School")

p1_data_long$edu_level <- factor(p1_data_long$edu_level, c("High School", 
                                                           "Vocational Education (DEP, ASP, AEP)", 
                                                           "Cegep (DEC, AEC)", 
                                                           "University (undergraduate, MA, PhD)", 
...
levels(p2_data_long$word_choice) <- c('Both', 'MUEV', 'Neither', 'MIEV')

p2_data_long %>% 
  mutate(word_choice = factor(word_choice, levels = c('Neither', 'Both','MUEV' , 'MIEV')),
  ...
  labs(x = 'Unintegrated Form / Integrated Form', y = 'Percentage', fill = 'Sentence choice') + 
  theme(axis.title.x = element_text(size = 15,face = 'bold', vjust = - 1),
        axis.title.y = element_text(size = 15,face = 'bold'),
        axis.text.y = element_text(size = 10),
        axis.text.x = element_text(size = 10, angle = -45, hjust = -0.05, vjust = 1.1))
```
...
rq2_data_encoded %>% 
  ...
  geom_violin() + 
  labs(x = 'Place Growing up', y = 'Probability of Using Unintegrated Forms', color = 'Place Growing up') + 
  theme(axis.title.x = element_text(size = 15,face = 'bold'),
        axis.title.y = element_text(size = 15,face = 'bold'),
        axis.text.y = element_text(size = 15),
        axis.text.x = element_text(size = 15))
...
rq2_data_encoded %>% 
  ...
  geom_violin() + 
  labs(x = 'Place of Residence', y = 'Probability of Using Unintegrated Forms', color = 'Place of Residence') + 
  theme(axis.title.x = element_text(size = 15,face = 'bold'),
        axis.title.y = element_text(size = 15,face = 'bold'),
        axis.text.y = element_text(size = 15),
        axis.text.x = element_text(size = 15))
...
```{r inno freq vs. education level, warning = FALSE}
rq2_data_encoded$edu_level <- recode_factor(rq2_data_encoded$edu_level,  
                                        `General and professional teaching college (DEC, AEC)` = "Cegep (DEC, AEC)", 
                                        `Professional training (DEP, ASP, AEP)` = "Vocational Education (DEP, ASP, AEP)", 
                                        `High school` = "High School")

rq2_data_encoded$edu_level <- factor(rq2_data_encoded$edu_level, c("High School", 
                                                           "Vocational Education (DEP, ASP, AEP)", 
                                                           "Cegep (DEC, AEC)", 
                                                           "University (undergraduate, MA, PhD)", 
                                                           'I prefer not to answer',
...
rq2_data_encoded %>% 
  ...
  geom_violin() + 
  labs(x = 'Education Level', y = 'Probability of Using Unintegrated Forms', color = 'Education Level') + 
  theme(axis.title.x = element_text(size = 15,face = 'bold'),
        axis.title.y = element_text(size = 15,face = 'bold'),
        axis.text.y = element_text(size = 15),
        axis.text.x=element_blank())
...
rq2_data_encoded %>% 
  ...
  geom_violin() + 
  labs(x = 'English Proficiency', y = 'Probability of Using Unintegrated Forms', color = 'English Proficiency') + 
  theme(axis.title.x = element_text(size = 15,face = 'bold'),
        axis.title.y = element_text(size = 15,face = 'bold'),
        axis.text.y = element_text(size = 15),
...
```
# Part 3: Age and Education

"I am pretty sure that the variables AGE and EDUCATION LEVEL are correlated: younger participants have a lower level of education and older participants have a higher level of education. That’s a problem, right? Is there a way to look at their correlation? In other similar studies, AGE is significant, but not EDUCATION LEVEL. But in my dataset, I don’t have young people with a high level of education and I don’t really have older people with a low level of education. " 


```{r age vs. edu (corr)}
data$edu_level <- recode_factor(
  data$edu_level,
  `General and professional teaching college (DEC, AEC)` = "Cegep (DEC, AEC)",
  `Professional training (DEP, ASP, AEP)` = "Vocational Education (DEP, ASP, AEP)",
  `High school` = "High School"
...
age_edu_data <- data %>% 
  select(age, edu_level) %>% 
  filter(!edu_level %in% c('I prefer not to answer','Primary school')) %>% 
  mutate(edu_encode = case_when(edu_level == 'High School'~1, 
                                edu_level == "Vocational Education (DEP, ASP, AEP)" ~ 2, 
                                edu_level == "Cegep (DEC, AEC)" ~ 3, 
                                edu_level == "University (undergraduate, MA, PhD)" ~4), 
         age_group = case_when(age < 20 ~ 1,
                               age >= 20 & age < 30 ~ 2,
                               age >= 30 & age < 40 ~ 3,
                               age >= 40 & age < 50 ~ 4,
                               age >= 50 ~ 5)) 
...
age_edu_data %>%
  summarize(
    corr_coef_ag = cor(age, edu_encode, method = "kendall", use = "complete.obs"),
    p_value = cor.test(age, edu_encode)[["p.value"]]
  )


age_edu_data %>%
  summarize(
    corr_coef_ag = cor(age_group, edu_encode, method = "kendall", use = "complete.obs"),
    p_value = cor.test(age_group, edu_encode)[["p.value"]]
  )
